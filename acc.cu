...
void acc(double **rho, std::vector<std::pair<std::string, std::vector<double>>> data, double *x_grid,
             double *z_grid, int N, int len_data, double alpha, int ll, int n_threads){
    
    double val = 0.;
    auto start = std::chrono::high_resolution_clock::now();
    // int n_threads = 3;
    // #pragma acc data copyout (data, x_grid, z_grid)
    // {
    // #pragma acc kernels
    {
    ...
    for (int i = 0; i < N; ++i)
        ...
        for (int j = 0; j < N; ++j)
        {
            rho[i][j] = 0.;
            val = 0.;
            #pragma acc kernels loop reduction(+:val) private(x_grid, data, z_grid)
            for (int k = 0; k < len_data; ++k)
            {
                val += data[3].second[k] * exp( -alpha*sqrt(pow(x_grid[i] - data[0].second[k], 2.) +
                                                                  pow(0         - data[1].second[k], 2.) + 
                                                                  pow(z_grid[j] - data[2].second[k], 2.) )  );
            }
            rho[i][j] = val;
    ...
    fout.open("logs.txt", std::fstream::app); //std::ios::app 
    if (!fout.is_open()) std::cout << "ошибка открытия logs";
    else fout<< "acc     " << "  alpha  " <<ll << " N   " << N << "   time    "<< durwrite.count() << std::endl;
    fout.close();
