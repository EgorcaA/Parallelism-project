...
#define THREADS_PER_BLOCK 625 //блок 25*25
...
__global__  void gpu(double *Dx_grid, double *Dz_grid, double *Drho, double *Dx_data, double *Dy_data, double *Dz_data, 
    ...
    // int i = ident/N;
    // int j = ident%N;
    Drho[threadRowID*N + threadColId] = 0.;
    for (int k = 0; k < len_data; ++k)
    {
        Drho[threadRowID*N + threadColId] += Ddensity_data[k] * exp( -alpha*sqrt(pow(Dx_grid[threadRowID] - Dx_data[k], 2.) +
                                                          pow(0          - Dy_data[k], 2.) + 
                                                          pow(Dz_grid[threadColId] - Dz_data[k], 2.) )  );
    }
...
int main(int argc, char const *argv[]) {
    ...
    double *x_data = (double*)  malloc ( sizeof(double)*len_data );
    cudaHostAlloc ( (void**) &x_data, sizeof(double)*len_data, cudaHostAllocDefault);
    for (int i = 0; i < len_data; ++i) x_data[i] = data[0].second[i];

    double *y_data = (double*)  malloc ( sizeof(double)*len_data );
    cudaHostAlloc ( (void**) &y_data, sizeof(double)*len_data, cudaHostAllocDefault);
    for (int i = 0; i < len_data; ++i) y_data[i] = data[1].second[i];

    double *z_data = (double*)  malloc ( sizeof(double)*len_data );
    cudaHostAlloc ( (void**) &z_data, sizeof(double)*len_data, cudaHostAllocDefault);
    for (int i = 0; i < len_data; ++i) z_data[i] = data[2].second[i];

    double *density_data = (double*)  malloc ( sizeof(double)*len_data );
    cudaHostAlloc ( (void**) &density_data, sizeof(double)*len_data, cudaHostAllocDefault);
    ...
    double *x_grid = (double*)  malloc ( sizeof(double)*N );
    cudaHostAlloc ( (void**) &x_grid, sizeof(double)*N, cudaHostAllocDefault);
    for (int i = 0; i < N; ++i) x_grid[i] = -R + (double)i/(N - 1)*2*R;

    double *z_grid = (double*)  malloc ( sizeof(double)*N );
    cudaHostAlloc ( (void**) &z_grid, sizeof(double)*N, cudaHostAllocDefault);
    for (int i = 0; i < N; ++i) z_grid[i] = -R + (double)i/(N - 1)*2*R;


    double *rho = (double*)  malloc ( sizeof(double)*N*N );
    cudaHostAlloc ( (void**) &rho, sizeof(double)*N*N, cudaHostAllocDefault);
    ...
    dim3 gridShape = dim3( (int)N/25, (int)N/25 );
    dim3 blockShape = dim3( 25, 25 );
    ...
    gpu <<< gridShape, blockShape  >>> (Dx_grid, Dz_grid, Drho, Dx_data, Dy_data, Dz_data, Ddensity_data, N, len_data, alpha, ll);
    ...
    printf ("\n 2D GPU with shared memory 5 workers calculation time %f msec\n", timerValueGPU);
